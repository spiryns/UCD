...
<div class="container" id="main">
      ...
      <div class="twin-card" id="twin-card" aria-hidden="true">
        ...
        <div class="twin-dial" id="twin-dial">
          <svg viewBox="0 0 200 200">
            ...
            <!-- 0° marker (links, want servo 0 = volledig links) -->
            <text x="22" y="106" fill="#666" font-size="12" font-weight="600">0</text>
            <text x="170" y="106" fill="#666" font-size="12" font-weight="600">180</text>
            <text x="92" y="22"  fill="#ffd166" font-size="13" font-weight="700">90</text>
            <!-- Naald: bij 90° wijst rechtop. transform=rotate(angle-90 100 100) -->
        ...
        <div class="twin-angle"><span id="twin-angle-val">90</span>°</div>
  ...
  <div id="page-settings" class="page" data-page-label="Instellingen.">
    ...
    <div class="setting-row">
      <h3>Trilling bij bocht</h3>
      <button class="setting-value" data-a11y data-action="cycleSetting" data-arg="vib_m9" id="val-vib_m9">
        <span class="cur">...</span>
        <span class="cycle-hint">▶</span>
      </button>
    </div>
...
let state = {
  ...
  settings: {
    ...
    vib_intensity:1, vib_m4:14, vib_m6:12, vib_m9:47,
...
async function twinPollOnce() {
  try {
    ...
    // Servo 0-180 mapt op SVG-naald rotatie -90 → +90 (naald wijst rechtop bij 90)
    if (needle) needle.setAttribute('transform', `rotate(${angle - 90} 100 100)`);
...
function handleCommand(cmd) {
  if (cmd === 'ASK_LOCATION') askLocation();
    ...
    if (num) {
      Speak.alert('Noodcontact wordt opgebeld');
